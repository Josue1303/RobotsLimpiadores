...
class RobotLimpieza(Agent):
    ...
    def estacion_carga_mas_cercana(self):
        ...
        estaciones = [agent.pos for agent in self.model.schedule.agents if isinstance(agent, EstacionCarga)]
        ...
        estacion_cercana = min(estaciones, key=lambda pos: self.distancia_euclidiana(self.pos, pos))
        print(f"Estación más cercana para el robot {self.unique_id} encontrada en {estacion_cercana}")
        return estacion_cercana

    def cagada_mas_cercana(self):
        cagadas_pos = [agent.pos for agent in self.model.schedule.agents if isinstance(agent, Celda) and agent.sucia]
    
        if not cagadas_pos:
            print("No hay cagadas en el modelo.")
            return None
        
        cagada_cercana = min(cagadas_pos, key=lambda pos: self.distancia_manhattan(self.pos, pos))
        print(f"Cagada más cercana para el robot {self.unique_id} encontrada en {cagada_cercana}")
    ...
    def a_star(self, start, goal):
        ...
        while open_list:
            ...
            for next_cell in self.model.grid.get_neighborhood(current, moore=True, include_center=False):
                if any(isinstance(agent, Mueble) for agent in self.model.grid.get_cell_list_contents([next_cell])):
    ...
    def step(self):
        ...
        vecinos = [vecino for vecino in vecinos if not isinstance(vecino, (Mueble, RobotLimpieza, EstacionCarga))]
        ...
        if any(isinstance(agent, EstacionCarga) for agent in self.model.grid.get_cell_list_contents([self.pos])):
            self.carga += 50
        ...
        if self.carga <= 40:
            print(f"Robot {self.unique_id} con carga {self.carga} buscando estación de carga...")
            estacion_pos = self.estacion_carga_mas_cercana()
    ...
    def mover_robots_a_estaciones(self):
        ...
        for robot in self.schedule.agents:
            if isinstance(robot, RobotLimpieza):
                estacion_cercana = min(estaciones, key=lambda pos: robot.distancia_euclidiana(robot.pos, pos))
